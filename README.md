...
- Implemented the complex sorting strategy with LSD binary radix sort and rank assignment.
...
- `complex_sort`: LSD binary radix sort
...
### How `--complex` works — LSD binary radix sort (O(n log n))

The `--complex` flag runs `radix_sort()` in `complex_sort.c`, implementing an LSD (Least Significant Digit) binary radix sort.
...
**Why O(n log n)?**
It is LSD binary radix sorting on the rank indices. There are O(log n) bit passes and each pass processes O(n) elements. So overall complexity is O(n · bits) = O(n log n).
